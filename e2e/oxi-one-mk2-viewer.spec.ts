...
test.describe('OXI ONE MKII Manual Viewer', () => {
  test.describe('Page Loading', () => {
    test('should load page 1', async ({ page }) => {
      const response = await page.goto(`${BASE_URL}${MANUAL_PATH}/1`);
    ...
    test('should load page 100 (middle)', async ({ page }) => {
      const response = await page.goto(`${BASE_URL}${MANUAL_PATH}/100`);
      expect(response?.status()).toBe(200);
    });

    test('should load page 272 (last)', async ({ page }) => {
      const response = await page.goto(`${BASE_URL}${MANUAL_PATH}/272`);
      expect(response?.status()).toBe(200);
    });

    test('should return 404 for page 0', async ({ page }) => {
      const response = await page.goto(`${BASE_URL}${MANUAL_PATH}/0`, {
        waitUntil: 'domcontentloaded',
      });
      expect(response?.status()).toBe(404);
    });

    test('should return 404 for page 273 (beyond last)', async ({ page }) => {
      const response = await page.goto(`${BASE_URL}${MANUAL_PATH}/273`, {
        waitUntil: 'domcontentloaded',
      });
      expect(response?.status()).toBe(404);
    });
  ...
  test.describe('Page Content', () => {
    test('should display page image on page 1', async ({ page }) => {
      await page.goto(`${BASE_URL}${MANUAL_PATH}/1`);
    ...
    test('should display correct page number in navigation', async ({ page }) => {
      await page.goto(`${BASE_URL}${MANUAL_PATH}/42`);
  ...
  test.describe('Keyboard Navigation', () => {
    test('should navigate to next page with ArrowRight', async ({ page }) => {
      ...
      // Wait for navigation
      await page.waitForURL(`${BASE_URL}${MANUAL_PATH}/2`);

      // Verify we're on page 2
      expect(page.url()).toBe(`${BASE_URL}${MANUAL_PATH}/2`);
    });

    test('should navigate to previous page with ArrowLeft', async ({ page }) => {
      await page.goto(`${BASE_URL}${MANUAL_PATH}/2`);

      // Press ArrowLeft
      await page.keyboard.press('ArrowLeft');

      // Wait for navigation
      await page.waitForURL(`${BASE_URL}${MANUAL_PATH}/1`);

      // Verify we're on page 1
      expect(page.url()).toBe(`${BASE_URL}${MANUAL_PATH}/1`);
    });
    ...
    test('should not navigate beyond last page with ArrowRight', async ({ page }) => {
      await page.goto(`${BASE_URL}${MANUAL_PATH}/272`);
      ...
      // Should still be on page 272
      expect(page.url()).toBe(`${BASE_URL}${MANUAL_PATH}/272`);
    });
  });

  test.describe('URL Structure (Backward Compatibility)', () => {
    test('existing URLs should work unchanged', async ({ page }) => {
      const testPages = [1, 10, 50, 100, 150, 200, 250, 272];

      for (const pageNum of testPages) {
        const response = await page.goto(`${BASE_URL}${MANUAL_PATH}/${pageNum}`);
        expect(response?.status()).toBe(200);
      }
    });

    test('URL pattern /manuals/oxi-one-mk2/page/{num} should work', async ({ page }) => {
      await page.goto(`${BASE_URL}/manuals/oxi-one-mk2/page/1`);
      expect(page.url()).toBe(`${BASE_URL}/manuals/oxi-one-mk2/page/1`);
    });
  });
  ...
  test.describe('Images Loading', () => {
    ...
    test('should load images from correct path', async ({ page }) => {
      await page.goto(`${BASE_URL}${MANUAL_PATH}/5`);
  ...
  test.describe('Navigation UI', () => {
    test('should show page navigation controls', async ({ page }) => {
      await page.goto(`${BASE_URL}${MANUAL_PATH}/10`);
    ...
    test('should navigate using prev/next buttons', async ({ page }) => {
      ...
      // Wait for navigation
      await page.waitForURL(`${BASE_URL}${MANUAL_PATH}/11`);
      expect(page.url()).toBe(`${BASE_URL}${MANUAL_PATH}/11`);
      ...
      // Wait for navigation
      await page.waitForURL(`${BASE_URL}${MANUAL_PATH}/10`);
      expect(page.url()).toBe(`${BASE_URL}${MANUAL_PATH}/10`);
    });
    ...
    test('should navigate using page selector', async ({ page }) => {
      ...
      // Wait for navigation
      await page.waitForURL(`${BASE_URL}${MANUAL_PATH}/50`);
      expect(page.url()).toBe(`${BASE_URL}${MANUAL_PATH}/50`);
